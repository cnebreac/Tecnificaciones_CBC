...
if show_admin_login:
    ...
    # >>> AVISO: recomendado abrir en Chrome + botones "Abrir en Chrome"
    ...
    <style>
      ...
      .cbc-close {{
        ...
        font-size: 16px; cursor: pointer; color: #7a5f00;
      }}
      .cbc-hidden {{ display: none; }}
    </style>
    
    <div id="cbc-banner" class="cbc-banner cbc-hidden">
      <button class="cbc-close" onclick="document.getElementById('cbc-banner').classList.add('cbc-hidden')">×</button>
      <h4>Para mejor funcionamiento, abre esta web en <strong>Google Chrome</strong>.</h4>
      <div>Si estás en otro navegador, usa uno de estos botones:</div>
      <div class="cbc-actions">
        <!-- Android (intent) -->
        <a class="cbc-btn" href="intent://tecnificacionescbc-oatzgiqulukgvrwtwhbpxe.streamlit.app/#Intent;scheme=https;package=com.android.chrome;end">
          Abrir en Chrome (Android)
        </a>
        <!-- iOS (esquema googlechrome://) -->
        <a class="cbc-btn" href="googlechrome://tecnificacionescbc-oatzgiqulukgvrwtwhbpxe.streamlit.app/">
          Abrir en Chrome (iPhone/iPad)
        </a>
        <!-- Enlace normal como respaldo -->
        <a class="cbc-btn" href="https://tecnificacionescbc-oatzgiqulukgvrwtwhbpxe.streamlit.app/" target="_blank" rel="noopener">
          Abrir en HTTPS
        </a>
      </div>
    </div>
    ...
    <script>
      (function () {{
        ...
        // Chrome en Android/iOS (CriOS en iOS). Excluir Edge ('Edg') y Opera ('OPR').
        const isChrome = (/Chrome|CriOS/).test(ua) && !(/Edg|OPR/).test(ua);
        // Si NO es Chrome, mostramos el banner
        if (!isChrome) {{
          document.getElementById('cbc-banner').classList.remove('cbc-hidden');
        }}
      }})();
    </script>
