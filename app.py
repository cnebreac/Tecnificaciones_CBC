...
def _with_backoff(fn):
    ...
    def _inner(*args, **kwargs):
        delays = [0, 0.5, 1.0, 2.0]  # reintentos rápidos para 429
        last_exc = None
        for d in delays:
            try:
                if d: time.sleep(d)
...
def _creds_cached_dict():
    # cacheamos el dict por si st.secrets es lento
    return dict(st.secrets["gcp_service_account"])

@st.cache_data(show_spinner=False, ttl=1800)
def _gc_cached():
    info = _creds_cached_dict()
    creds = Credentials.from_service_account_info(info, scopes=SCOPES)
    return gspread.authorize(creds)

@st.cache_data(show_spinner=False, ttl=1800)
def _open_sheet_cached(sheet_id: str):
    gc = _gc_cached()
...
def _open_sheet():
    ...
    if not sheet_id:
        ...
        st.stop()
    try:
        return _open_sheet_cached(sheet_id)
    except gspread.exceptions.APIError as e:
...
def _read_ws_as_df(sheet_name: str) -> pd.DataFrame:
    sh = _open_sheet()
    ...
    for c in _EXPECTED_HEADERS:
        if c not in df.columns:
            df[c] = ""
    return df[_EXPECTED_HEADERS].copy()

def load_df(sheet_name: str) -> pd.DataFrame:
    # wrapper por compatibilidad
    return _read_ws_as_df(sheet_name)

# ====== SESIONES ======
SESIONES_SHEET = "sesiones"

@st.cache_data(show_spinner=False, ttl=30)
def _load_sesiones_raw() -> pd.DataFrame:
    """Lee la pestaña sesiones con cache y normaliza columnas."""
    sh = _open_sheet()
    # Intentar abrir sin crear
    try:
        ws = sh.worksheet(SESIONES_SHEET)
    except Exception:
        # si no existe, crearla una vez
        try:
            ...
            if "already exists" in str(e).lower() or "already exists" in getattr(e, "message", "").lower():
                ws = sh.worksheet(SESIONES_SHEET)
            else:
                raise
    vals = _ws_get_all_values(ws)
    ...
    return df[["fecha_iso","hora","estado"]].copy()

def _ensure_ws_sesiones_for_write():
    """Obtiene el worksheet 'sesiones' para ESCRIBIR, sin lecturas extra."""
    sh = _open_sheet()
    try:
        return sh.worksheet(SESIONES_SHEET)
    except Exception:
        try:
            ws = sh.add_worksheet(title=SESIONES_SHEET, rows=100, cols=3)
            ws.update("A1:C1", [["fecha_iso","hora","estado"]])
...
def load_sesiones_df() -> pd.DataFrame:
    return _load_sesiones_raw()

def get_sesiones_por_dia(pre_df: pd.DataFrame | None = None) -> dict:
    df = pre_df if pre_df is not None else load_sesiones_df()
    out = {}
...
def get_sesion_info(fecha_iso: str, hora: str, pre_df: pd.DataFrame | None = None) -> dict:
...
def upsert_sesion(fecha_iso: str, hora: str, estado: str = "ABIERTA"):
    ws = _ensure_ws_sesiones_for_write()
    rows = _ws_get_all_values(ws)
    if not rows:
        ws.update("A1:C1", [["fecha_iso","hora","estado"]])
        rows = _ws_get_all_values(ws)
    ...
    for i, row in enumerate(rows[1:], start=2):
        if len(row) >= 2 and _norm_fecha_iso(row[0]) == f_iso and _parse_hora_cell(row[1]) == hora_n:
            ...
            st.cache_data.clear()  # invalidar caches
            return
...
def set_estado_sesion(fecha_iso: str, hora: str, estado: str):
    ...
    for i, row in enumerate(rows[1:], start=2):
        if len(row) >= 2 and _norm_fecha_iso(row[0]) == f_iso and _parse_hora_cell(row[1]) == hora_n:
            ws.update_cell(i, 3, estado.upper()); st.cache_data.clear(); return

# ====== LECTURA INSCRIPCIONES/WAITLIST (PRE-CARGA / SIN LECTURAS EN BUCLE)
...
@st.cache_data(show_spinner=False, ttl=30)
def _preload_dataframes():
    ins = _prep_df_reservas(load_df("inscripciones"))
    wl  = _prep_df_reservas(load_df("waitlist"))
    ses = load_sesiones_df()
    return ins, wl, ses

def get_inscripciones_por_sesion(fecha_iso: str, hora: str, ins_df: pd.DataFrame) -> list:
    f_iso = _norm_fecha_iso(fecha_iso); h_n = _parse_hora_cell(hora)
    m = ins_df[(ins_df["fecha_iso"] == f_iso) & (ins_df["hora"] == h_n)]
    return m.to_dict("records")

def get_waitlist_por_sesion(fecha_iso: str, hora: str, wl_df: pd.DataFrame) -> list:
    f_iso = _norm_fecha_iso(fecha_iso); h_n = _parse_hora_cell(hora)
    m = wl_df[(wl_df["fecha_iso"] == f_iso) & (wl_df["hora"] == h_n)]
    return m.to_dict("records")

def _match_canasta(valor: str, objetivo: str) -> bool:
    v = (valor or "").strip().lower()
    o = objetivo.strip().lower()
    if o.startswith("mini"):    return v.startswith("mini")
    if o.startswith("canasta"): return v.startswith("canasta")
    return v == o

def plazas_ocupadas(fecha_iso: str, hora: str, canasta: str, ins_df: pd.DataFrame) -> int:
    ins = get_inscripciones_por_sesion(fecha_iso, hora, ins_df)
    return sum(1 for r in ins if _match_canasta(r.get("canasta",""), canasta))

def plazas_libres(fecha_iso: str, hora: str, canasta: str, ses_df: pd.DataFrame, ins_df: pd.DataFrame) -> int:
    if get_sesion_info(fecha_iso, hora, ses_df).get("estado","ABIERTA").upper() == "CERRADA":
        return 0
    return max(0, MAX_POR_CANASTA - plazas_ocupadas(fecha_iso, hora, canasta, ins_df))

def ya_existe_en_sesion(fecha_iso: str, hora: str, nombre: str, ins_df: pd.DataFrame, wl_df: pd.DataFrame) -> str | None:
    nn = _norm_name(nombre)
    for r in get_inscripciones_por_sesion(fecha_iso, hora, ins_df):
        if _norm_name(r.get("nombre","")) == nn:
            return "inscripciones"
    for r in get_waitlist_por_sesion(fecha_iso, hora, wl_df):
        if _norm_name(r.get("nombre","")) == nn:
            return "waitlist"
    return None

# ====== APPEND GENÉRICO (sin cache)
...
def crear_pdf_sesion(fecha_iso: str, hora: str, ins_df: pd.DataFrame, wl_df: pd.DataFrame) -> BytesIO:
    from reportlab.lib.pagesizes import A4
    from reportlab.pdfgen import canvas
    from reportlab.lib.units import cm
    ...
    hora = _parse_hora_cell(hora)
    lista = get_inscripciones_por_sesion(fecha_iso, hora, ins_df)
    wl    = get_waitlist_por_sesion(fecha_iso, hora, wl_df)
    buf = BytesIO(); c = canvas.Canvas(buf, pagesize=A4)
    width, height = A4
    fecha_txt = d.strftime("%A, %d %B %Y").capitalize()
    y = height - 2*cm
    c.setFont("Helvetica-Bold", 16); c.drawString(2*cm, y, f"Tecnificación Baloncesto — {fecha_txt} {hora}")
...
# ========= PRELOAD DATA (una sola vez por run / TTL 30s) =========
INS_DF, WL_DF, SES_DF = _preload_dataframes()
    ...
    if not st.session_state.is_admin:
        ...
        if df_ses_all.empty:
            ...
            if df_ses_listables.empty:
                ...
                opciones = {(f,h): f"{dt.datetime.strptime(f,'%Y-%m-%d').strftime('%d/%m/%Y')}  ·  {h}  ·  {get_sesion_info(f,h, SES_DF).get('estado','—')}"
                ...
                ins_f = get_inscripciones_por_sesion(f_sel, h_sel, INS_DF)
                wl_f  = get_waitlist_por_sesion(f_sel, h_sel, WL_DF)
                st.write("**Inscripciones:**")
                st.dataframe(pd.DataFrame(ins_f) if ins_f else pd.DataFrame(columns=["—"]), use_container_width=True)

                st.write("**Lista de espera:**")
                st.dataframe(pd.DataFrame(wl_f) if wl_f else pd.DataFrame(columns=["—"]), use_container_width=True)

                if st.button("🧾 Generar PDF (inscripciones + lista de espera)"):
                    try:
                        pdf = crear_pdf_sesion(f_sel, h_sel, INS_DF, WL_DF)
                        st.download_button(
                            label="Descargar PDF",
                            data=pdf,
...
""")
    ...
    # Sesiones pre-cargadas (sin lecturas nuevas)
    SESIONES_DIA = get_sesiones_por_dia(SES_DF)
    today = dt.date.today()
    ...
    # Calendario ligero (sin consultas por cada evento)
    try:
        ...
        for f, sesiones in SESIONES_DIA.items():
            ...
            if fecha_dt < today:
                ...
                if not any_abierta:
                    ...
                    for s in sesiones:
                        mm = plazas_libres(f, s["hora"], CATEG_MINI, SES_DF, INS_DF)
                        gg = plazas_libres(f, s["hora"], CATEG_GRANDE, SES_DF, INS_DF)
        ...
        cal = calendar(events=events, options={"initialView":"dayGridMonth","height":600,"locale":"es","firstDay":1}, key="cal")
    ...
    info_s = get_sesion_info(fkey, hkey, SES_DF)
    ...
    libres_mini = plazas_libres(fkey, hkey, CATEG_MINI, SES_DF, INS_DF)
    libres_gran = plazas_libres(fkey, hkey, CATEG_GRANDE, SES_DF, INS_DF)
...
*(En **lista de espera** también se genera justificante, identificado como “Lista de espera”.)*
    ...
    if st.session_state.get(ok_flag):
        ...
        with placeholder.form(f"form_{fkey}_{hkey}", clear_on_submit=True):
            ...
            # Forzar Minibasket si es Benjamín/Alevín
            ...
            if enviar:
                ...
                if errores:
                    ...
                    ya = ya_existe_en_sesion(fkey, hkey, nombre, INS_DF, WL_DF)
                    if ya == "inscripciones":
                        ...
                        libres_cat = plazas_libres(fkey, hkey, final_canasta, SES_DF, INS_DF)
                        row = [
...
# ====== DIAGNÓSTICO (manual, no se ejecuta salvo click)
...
Si `trashed` = true → está en papelera.
        """)
    else:
        ...
        st.info("""
• 404 → ID mal o archivo movido/borrado.
• 403 → sin permisos (añade la service account a la **Unidad compartida**).
        """)
